 ...
 * <p>
 * A Map will allow retrieval of an item by its key, a list of all keys, the key corresponding to an item, whether that item exists, etc.
 * 
...
public interface IMap<K,V> extends Iterable<IPair<K,V>>, IHasTwoGenericParams<K,V>, ICloneable<IMap<K,V>> {	
	 ...
	 * @return True, if and only if every item which is present in items is a value present in this Map
	 ...
	 * @return True, if and only if every key in this Map is present in map, and if every key in this Map corresponds to the same value in map
	 ...
	 * @return	Returns the entity with the associated key. (If it isn't in the Map, it returns null.)
	 * @throws IllegalArgumentException If the provided key is null, a blank String, etc. Additionally, if this Map is only expecting a limited set of keys, and key is not one of those keys, this exception will be thrown.
	 * @throws IllegalStateException If this Map is a Set of entities with keys, and the entity obtained by this method does not have the same key as the parameter provided to this method, this exception will be thrown. Additionally, if this Map is expecting a value for every key requested, and a value does not exist for the requested key, this exception will be thrown. 
	 */
	 ...
	 * @return The keys of the item in this map. (If it isn't in the Map, it returns an empty collection.)
	 */
	 ...
	 * @return The value, if and only if the item was removed (and therefore if it was present); else, null
	 */
	V removeByKey(K key);
	...
	/**
	 * Sets a function used to validate all inputs to this Map. (If this is null, then all inputs are valid.)
	 * <i>Intended usage is that if the input passes validation, the validator returns null; otherwise, the validator returns the exception message.</i>
