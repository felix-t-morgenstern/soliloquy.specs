...
public interface ISettingsRepo extends IEntityGroup<ISetting>, IHasId, IReads, IWrites {
	 ...
	 * <p>
	 * <i>It is expected that any code accessing a Setting will know what type of value that Setting will provide. (The underlying assumption is that Settings will only be added and altered by code from the component that placed it in the SettingsRepo in the first place.)</i>
	 ...
	 * @throws IllegalArgumentException If and only if settingId is null or empty, there is no setting with the specified Id and with the same type as the provided Value, or if the value being assigned to this Setting is illegal
	 */
	 ...
	 * @param order The order in this SettingsRepo in which the Setting appears. (Order numbers do not need to be positive, consecutive, etc; they must be unique.)
	 * @throws IllegalArgumentException If order is already used within this SettingsRepo; or if setting is null; or if setting's Id is already present in this SettingsRepo or any of its children; or if groupId is non-empty, non-null, and does not exist within this group
	 */
	void addEntity(ISetting setting, int order, String groupId) throws IllegalArgumentException;
	
	@Override
	/**
	 * <i>This method requires that all values read from the data already exist in this SettingsRepo</i>
	 * <p>
	 ...
	 * @throws IllegalArgumentException If the data are illegal, e.g. if they are null, if they are not properly formatted, etc., or if any of the values in the data do not exist in the SettingsRepo
	 */
